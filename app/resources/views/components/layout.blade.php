...
<body class="min-h-screen text-gray-900" style="{{ $isAuth ? 'background-image:url('.asset('img/wall_A.jpg').');background-size:cover;background-position:center' : '' }}">
    ...
    <div class="max-w-5xl mx-auto p-4 relative">
        <div x-data="{show:false,text:'',type:'success',modal:false,modalTitle:'',modalText:'',confirmShow:false,confirmTitle:'',confirmText:'',okLabel:'Aceptar',cancelLabel:'Cancelar',confirmResolve:null}"
             x-init="window.notify=(t,k='success')=>{if(!t)return;text=t;type=k;show=true;setTimeout(()=>show=false,3000)};window.showModal=(title,txt)=>{modalTitle=title;modalText=txt;modal=true};window.confirmModal=(title,txt,opts={})=>new Promise((resolve)=>{confirmTitle=title;confirmText=txt;okLabel=opts.ok||'Aceptar';cancelLabel=opts.cancel||'Cancelar';confirmResolve=resolve;confirmShow=true})">
            ...
            <div x-show="modal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center">
                ...
                <div class="bg-white rounded shadow p-4 z-10 w-96">
                    ...
                    <p class="mb-4" x-text="modalText"></p>
